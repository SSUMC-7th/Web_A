...
const fetchRecommendedMovies = async ({ queryKey }) => {
  const [_, movieId] = queryKey;
  const apiKey = import.meta.env.VITE_API_KEY;
  const apiUrl = import.meta.env.VITE_MOVIE_API_URL;
  const response = await fetch(`${apiUrl}/movie/${movieId}/recommendations?api_key=${apiKey}&language=ko-KR`);
...
const MovieDetailPage = () => {
  ...
  const { data: recommendedMoviesData, isLoading: recommendedLoading, isError: recommendedError } = useQuery({
    queryKey: ['recommendedMovies', movieId],
    queryFn: fetchRecommendedMovies,
  });

  if (movieLoading || creditsLoading || recommendedLoading) return <h1 style={{ color: 'white' }}>로딩중...</h1>;
  if (movieError || creditsError || recommendedError) return <h1 style={{ color: 'white' }}>에러 발생</h1>;

  if (!movie || !creditsData || !recommendedMoviesData) return null;
...
const RecommendedMoviesSection = styled.div`
  margin-top: 50px;
